...
<table class="table table-bordered nowrap responsive datatables" cellspacing="0" id="ett_conclusion_tbl" width="100%" >
   <thead>
        <tr>
        ...
        <tr id="<?php echo $key['id']; ?>">
            ...
            <?php if($loggedin_user['is_admin']==1){ ?>
                <td class="exam_cate" onClick="showExamination(this);">
                    <input type="text" class="form-control border-0 bg-transparent shadow-none" name="ett_conclusion" value="<?php echo $key['conclusion']; ?>" onchange="saveToconclusion(this,'conclusion','<?php echo $key['id']; ?>')">        
                </td>
...
<script type="text/javascript">
    $("#ett_conclusion_tbl tbody tr").click(function (e) {
            $('#ett_conclusion_tbl tbody tr.row_selected').removeClass('row_selected');
...
$(document).ready(function () {
    // Sortable rows
    table = $("#ett_conclusion_tbl");
    table.tableDnD({
        onDrop: function(table, row) {
            var rows = table.tBodies[0].rows;
            var tabledata = $.tableDnD.serialize();
            var tblname = 'ett_conclusion';
            var tblid = 'id';
           $.ajax({
                url: window.location.origin+window.location.pathname+"setting/sort_ett_conclusion_tbl/"+tblname+"/"+tblid,
                type: 'post',
                data: tabledata,
                cache: false,
                success: function(response){
